...
import { estilos, CORES_PRIORIDADE } from '../styles/theme'
...
function ITUserDetail({ usuario, versaoDados, onVoltar, onSelect }) {
  ...
  return (
    <div className="animate-fade-up">
      <button onClick={onVoltar} style={{ background: 'none', border: 'none', color: '#7b92b4', cursor: 'pointer', fontSize: 14, marginBottom: 24, display: 'flex', alignItems: 'center', gap: 6, padding: 0, fontFamily: 'Inter, sans-serif' }}>
        ← Colaboradores
      </button>
      <div style={{ ...estilos.card, border: '1px solid rgba(0,120,81,0.14)', borderRadius: 16, padding: '20px 24px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 18, flexWrap: 'wrap' }}>
        <div style={{ width: 58, height: 58, borderRadius: '50%', background: usuarioAtual.emAguardoDeCadastro ? 'rgba(255,255,255,0.08)' : '#007851', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Outfit, sans-serif', fontWeight: 700, fontSize: 22, color: '#fff', flexShrink: 0 }}>
          {usuarioAtual.emAguardoDeCadastro ? '?' : obterIniciais(usuarioAtual.name)}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          {usuarioAtual.emAguardoDeCadastro ? (
            <>
              <h1 style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 800, fontSize: 20, color: '#f0f4ff', margin: '0 0 6px' }}>Conta resetada</h1>
            ...
            <>
              <h1 style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 800, fontSize: 20, color: '#f0f4ff', margin: '0 0 3px' }}>{usuarioAtual.name}</h1>
              <p style={{ color: '#7b92b4', fontSize: 14, margin: '0 0 2px' }}>{usuarioAtual.role} · {usuarioAtual.dept}</p>
            </>
          )}
          <p style={{ color: '#4a5f7a', fontSize: 12, margin: 0 }}>{usuarioAtual.email}</p>
        </div>
        ...
        <div style={{ ...estilos.card, border: '1px solid rgba(245,158,11,0.2)', padding: '16px 18px', marginBottom: 26, display: 'flex', flexDirection: 'column', gap: 10 }}>
          {!confirmandoReset ? (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
              <div>
                <div style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 600, fontSize: 13, color: '#f0f4ff' }}>Este e-mail mudou de responsável?</div>
                <div style={{ color: '#7b92b4', fontSize: 12, marginTop: 2 }}>Resetar apaga nome, cargo e senha atuais — a próxima pessoa completa o Primeiro Acesso do zero.</div>
              </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div style={{ color: '#f0f4ff', fontSize: 13 }}>
              ...
              {erroReset && <p style={{ color: '#f87171', fontSize: 13, margin: 0 }}>{erroReset}</p>}
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button onClick={() => { setConfirmandoReset(false); setErroReset('') }} disabled={resetando}
                  style={{ background: 'rgba(255,255,255,0.05)', color: '#7b92b4', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: '9px 16px', fontSize: 13, fontFamily: 'Outfit, sans-serif', fontWeight: 500, cursor: 'pointer' }}>
                  Cancelar
                </button>
              </div>
            </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          {chamados.map(chamado => (
            <div key={chamado.id} onClick={() => onSelect(chamado)}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 600, fontSize: 15, color: '#f0f4ff', marginBottom: 3 }}>{chamado.summary}</div>
                <div style={{ color: '#7b92b4', fontSize: 12 }}>{formatarData(chamado.created)} · {LABEL_CATEGORIA[chamado.category]}</div>
              </div>
